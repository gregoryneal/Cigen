...
namespace System.Collections.Generic
{
    ...
    public class PriorityQueue<TElement, TPriority>
    {
        ...
        /// <summary>
        /// Returns the first N minimal elements from the <see cref="PriorityQueue{TElement, TPriority}"/> without removing them.
        /// </summary>
        /// <param name="num"></param>
        /// <returns></returns>
        public IEnumerable<(TElement, TPriority)> Peek(int num) {
            if (_size == 0) {
                throw new InvalidOperationException();
            }

            if (num > _size) num = _size;

            return _nodes.Take(num);
        }
        ...
        private void Grow(int minCapacity)
        {
            ...
            if ((uint)newcapacity > maxCapacity) newcapacity = maxCapacity;
